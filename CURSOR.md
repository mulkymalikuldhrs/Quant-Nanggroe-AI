...
## Pipeline Status
```
✅ WIRED:   7-stage pipeline, 8 scorers, MT5 live, 1079 providers
🔴 BROKEN:  Evolution loop (4 bugs in main.py:847-854)
🗄️ EXIST:   453+ alpha factors (engine/factors/), 10 REST clients
```

## Key Paths
- `quant_nanggroe/hedge_fund/portfolio/main.py` — pipeline hub
- `quant_nanggroe/core/scoring/` — 8 scorers + FusionEngine
- `quant_nanggroe/engine/evolution/` — 🔴 broken evolution loop
- `quant_nanggroe/providers/` — HiddenRegime + News (3-tier)
- `quant_nanggroe/api/routes/evolution.py` — evolution API

**Index:** Include `quant_nanggroe/`, `dashboard/src/`, `docs/`, `qna.py`
