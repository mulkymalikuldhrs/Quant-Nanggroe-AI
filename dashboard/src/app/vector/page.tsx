...
type Mispricing = { d: number; threshold: number; is_trigger: boolean; reason?: string };
type VectorStatus = {
  manifold: Record<string, number[]>;
  mispricing: Record<string, Mispricing>;
  warming_up?: boolean;
  p0_source?: string;
  history_len?: number;
};
...
export default function VectorPage() {
  const [data, setData] = useState<VectorStatus | null>(null);
  useEffect(() => {
    apiRequest<VectorStatus>("/api/vector/status").then(setData).catch(() => {});
  }, []);
  const warmingUp = data?.warming_up === true;
  ...
  return (
    <div className="space-y-4 animate-slide-up">
      ...
      {warmingUp && (
        <p className="text-sm text-amber-300/90">Warming up ({historyLen}/20) — collecting rolling history before mispricing can trigger.</p>
      )}
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <ChartCard title="Manifold Points" subtitle={data?.p0_source ? `3D projection (x USD, y EUR, z EURUSD) • p0=${data.p0_source} • n=${historyLen}` : "3D projection (x USD, y EUR, z EURUSD)"}>
        ...
        <ChartCard title="Euclidean Mispricing" subtitle="d > threshold (box merah)">
          {data ? Object.entries(data.mispricing).map(([k, v]) => (
            <div key={k} className="flex justify-between py-1 text-xs" title={v.reason ?? (warmingUp ? "warming up" : "ok")}><span className="text-white/50">{k}</span><span className={v.is_trigger?"text-red-400":"text-emerald-400"}>d={v.d.toFixed(4)} thr={v.threshold.toFixed(4)} {v.is_trigger?"TRIGGER":"OK"}{v.reason ? ` • ${v.reason}` : ""}</span></div>
