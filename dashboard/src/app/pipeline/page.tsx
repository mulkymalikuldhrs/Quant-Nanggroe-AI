...
const PIPELINE_COMPONENTS = [
  ...
  },
  {
    ...
    stage: 3,
    icon: Brain,
    description: "20 AI agents vote on market direction — consensus override when confidence < 0.6",
    status: "operational",
  ...
  },
  {
    ...
    stage: 4,
    icon: TrendingUp,
    description: "hedge_fund.py's 10 core signal providers — weighted voting from standalone system",
    status: "operational",
  ...
  },
  {
    ...
    stage: 5,
    icon: GitBranch,
    description: "Auto-discover strategies from registry + MUE-X evolved genes via GeneLoader",
    status: "operational",
  ...
  },
  {
    ...
    stage: 6,
    icon: Filter,
    description: "Filters strategy candidates by market regime compatibility (min 0.35 threshold)",
    status: "operational",
    config: { min_compatibility: 0.35, regime_map: "REGIME_STRATEGY_MAP", enabled: true },
  ...
  },
  {
    ...
    stage: 7,
    icon: Users,
    description: "Regime-weighted voting across strategies — momentum, mean-reversion, volatility, pattern",
    status: "operational",
  ...
  },
  {
    ...
    stage: 8,
    icon: Users,
    description: "Multi-agent council debate when confidence < threshold — simulated 50-council vote",
    status: "operational",
  ...
  },
  {
    ...
    stage: 9,
    icon: Shield,
    description: "9-gate RiskManager: KillSwitch, CooldownGuard, MaxPositionGuard, ATR sizing, Leverage",
    status: "operational",
    config: {
      kill_switch: true,
  ...
  },
  {
    ...
    stage: 10,
    icon: ShieldAlert,
    description: "One Final Veto — regime + portfolio + risk + kelly + SL/TP placement combined decision",
    status: "operational",
    config: {
      min_confidence: 0.6,
      min_rr_ratio: 2.5,
      kelly_fraction: 0.25,
      min_regime_compat: 0.35,
    },
  ...
  },
  {
    ...
    stage: 11,
    icon: Terminal,
    description: "MT5 live execution or Paper mode — order routing, fill tracking, SL/TP placement",
    status: "operational",
  ...
  },
  {
    ...
    stage: 12,
    icon: ScrollText,
    description: "Logs EVERY triggered strategy — symbol, action, regime, confidence, SL/TP, ATR",
    status: "operational",
  ...
  },
  {
    ...
    stage: 13,
    icon: DollarSign,
    description: "Closed-PnL evaluation — win rate, Sharpe, drawdown check → fine-tune trigger",
    status: "operational",
    config: {
      stats_dir: "data/strategy_stats",
      min_trades_for_eval: 3,
      fine_tune_win_rate: 0.4,
      fine_tune_sharpe: 0.5,
    },
  ...
  },
  {
    ...
    stage: 14,
    icon: RefreshCw,
    description: "Self-correcting loop — bad strategies get fine-tuned, good ones reinforced, lessons recorded",
    status: "operational",
  ...
  {
    id: "hf_standalone",
    name: "Hedge Fund (Standalone)",
    stage: 15,
    icon: Cpu,
    description: "Original 6,693-line standalone system — now bridged as signal provider for QNA",
    status: "archived",
...
function PipelineCard({
  ...
  component: (typeof PIPELINE_COMPONENTS)[number];
  index: number;
}) {
  ...
  return (
    <div
      className={`group rounded-xl border ${statusBg} backdrop-blur-sm transition-all duration-300 hover:border-white/20 hover:bg-white/[0.04]`}
    >
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2">
            ...
            <div className={`flex-shrink-0 w-1.5 h-1.5 rounded-full ${component.status === "operational" ? "bg-emerald-400" : "bg-gray-500"}`} />
          </div>
        ...
        <div className="flex items-center gap-3 flex-shrink-0">
          <span className={`text-[10px] uppercase tracking-wider font-medium ${statusColor}`}>
            {component.status}
          </span>
          {expanded ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-3">
                  ...
                  <div key={key} className="flex items-center justify-between py-1">
                    ...
                    <div className="flex items-center gap-2">
                      {typeof value === "boolean" ? (
                        <button
                          onClick={() => toggleConfig(component.id, key)}
                          className={`w-8 h-4 rounded-full transition-colors duration-200 cursor-pointer ${
                            componentConfigs[component.id]?.[key] ?? value ? "bg-emerald-500" : "bg-white/[0.1]"
                          } relative`}
                        >
                          <div
                            className={`absolute top-0.5 w-3 h-3 rounded-full bg-white transition-transform duration-200 ${
                              componentConfigs[component.id]?.[key] ?? value ? "translate-x-4" : "translate-x-0.5"
                            }`}
                          />
                        </button>
                      ) : (
                        <span className="text-xs text-white/70 font-mono bg-white/[0.04] px-2 py-0.5 rounded">
                          {String(value)}
                        </span>
                      )}
                    </div>
            ...
            <div>
              ...
              <div className="mt-3 flex gap-2">
                <button
                  onClick={() => console.log(`Run ${component.id} for symbol BTC-USD`)}
                ...
                <button
                  onClick={() => console.log(`Open config for ${component.id}:`, { ...component.config, ...componentConfigs[component.id] })}
                ...
                <button
                  onClick={() => console.log(`Fetch logs for ${component.id}`)}
      ...
      {/* Bottom connector line between stages */}
...
export default function PipelinePage() {
  ...
  const [filter, setFilter] = useState<"all" | "operational" | "degraded">("all");
  const [time, setTime] = useState(new Date());
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [componentConfigs, setComponentConfigs] = useState<Record<string, Record<string, boolean>>>({});
  const [liveData, setLiveData] = useState<typeof PIPELINE_COMPONENTS | null>(null);
  ...
  const fetchPipelineStatus = useCallback(async () => {
    try {
      ...
      if (data?.stages && Array.isArray(data.stages)) {
        setLiveData(data.stages);
      }
      setError(null);
    } catch (err) {
      // Fall back to hardcoded data silently
      console.debug("Pipeline API fetch failed, using fallback data:", err);
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  return (
    <div className="space-y-6 animate-fade-in">          {/* Header */}
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="text-sm text-white/40 mt-1">
            X·Y·Z Autonomous Hedge Fund Pipeline — {components.length} stages, {components.filter((c) => c.status === "operational").length} operational
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            className={`p-2 rounded-lg transition-colors duration-200 ${
              autoRefresh ? "bg-emerald-500/10 text-emerald-400" : "bg-white/[0.04] text-white/40"
            }`}
          >
      ...
      {/* Filter + Overview Stats */}
      <div className="flex items-center gap-2 mb-2">
        {(["all", "operational", "degraded"] as const).map((f) => (
          <button
            key={f}
            onClick={() => setFilter(f)}
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          ...
          { label: "Operational", value: components.filter((c) => c.status === "operational").length, icon: CheckCircle2, color: "text-emerald-400" },
          { label: "Degraded", value: components.filter((c) => c.status === "degraded").length, icon: AlertTriangle, color: "text-amber-400" },
          { label: "Pipeline Health", value: `${Math.round((components.filter((c) => c.status === "operational").length / components.length) * 100)}%`, icon: Activity, color: "text-violet-400" },
        ].map((stat) => (
          <div
            key={stat.label}
            className="rounded-xl bg-white/[0.03] border border-white/[0.06] p-4 backdrop-blur-sm hover:border-white/10 transition-colors duration-200"
          >
            <div className="flex items-center gap-3">
      ...
      <div className="rounded-xl bg-gradient-to-br from-white/[0.02] to-transparent border border-white/[0.06] p-6 backdrop-blur-sm">
        ...
        <div className="flex flex-wrap items-center gap-2">
          {components.slice(0, 14).map((comp, i) => (
