...
from quant_nanggroe_ai.types import NewsEventType
...
class SentimentTool:
    ...
    async def _fetch_alpha_vantage_news(self, symbol: str) -> list[dict[str, str]]:
        ...
        api_key = self._settings.data_sources.alpha_vantage_api_key
        if not api_key:
        ...
        req = urllib.request.Request(url, headers={"User-Agent": "QuantNanggroeAI/1.0"})
    ...
    async def _fetch_polygon_news(self, symbol: str) -> list[dict[str, str]]:
        ...
        api_key = self._settings.data_sources.polygon_api_key
        if not api_key:
