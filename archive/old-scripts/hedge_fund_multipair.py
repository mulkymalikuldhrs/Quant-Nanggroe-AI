...
# ── Strategy Config ──
# (registry_name, params, display_label, weight_multiplier)
BEST_STRATEGIES = [
    ("WyckoffStrategy",          {"lookback": 50, "volume_mult": 1.3},   "Wyckoff",   1.0),
    ("MeanReversionStrategy",    {"k_period": 14, "d_period": 5,
                                   "oversold": 25, "overbought": 75},    "MeanRev",   0.85),
]

# ── Helpers ──

def compute_atr(symbol, period=14, tf=mt5.TIMEFRAME_M15):
    """Quick ATR for SL/TP sizing"""
...
def calc_lot(balance, confidence):
    """Dynamic lot sizing based on balance + confidence"""
...
def run_signal_on_pair(symbol, strategy_name, strategy_params, bars=300):
    """Run MTF signal for one pair with one strategy across all 5 styles.
    Returns best signal or None if neutral."""
...
def evaluate_all_pairs(valid_pairs):
    """Run all strategies on all valid pairs → return ranked candidate list."""
    candidates = []
    ...
    for pair in valid_pairs:
        ...
        for strat_name, strat_params, strat_label, strat_weight in BEST_STRATEGIES:
            ...
            # Combine base MTF confidence with strategy weight
            ...
            log.info(f"  📊 {symbol:8s} {strat_label:12s} {sig['bias']:5s} "
                      f"conf={combined:.2f} style={sig['style']}")
...
def execute_best(candidate, balance):
    """Place MT5 order for the top-ranked candidate."""
    ...
    if res and res.retcode == 10009:
        ...
        log_trade(f"open_{bias}", symbol, lot, price, sl, tp, atr_val,
                  f"executed (order={res.order})",
                  pnl="")
        return True
    else:
        ...
        log_trade(f"fail_{bias}", symbol, lot, price, sl, tp, atr_val,
                  f"code={ret}")
        return False
...
def run_multipair_cycle():
    """Full pipeline: scan → evaluate → execute best → log."""
    log.info("══════════════════════════════════════════")
    ...
    # ── 2. Scan all pairs (spread filter) ──
    ...
    # ── 4. Evaluate all strategies × all pairs ──
    ...
    # ── 5. Rank and execute best ──
    best = candidates[0]
    log.info(f"\n🏆 TOP SIGNAL: {best['symbol']:8s} {best['bias'].upper():5s}  "
             f"conf={best['confidence']:.2f}  "
             f"strategy={best['strategy']}  style={best['style']}")
    ...
    # ── 6. Log summary of all candidates ──
    log.info(f"\n── Full ranking ({len(candidates)} candidate(s)) ──")
    for i, c in enumerate(candidates, 1):
        log.info(f"  #{i} {c['symbol']:8s} {c['bias']:5s}  "
                 f"conf={c['confidence']:.2f}  {c['strategy']:12s}  {c['style']}")
