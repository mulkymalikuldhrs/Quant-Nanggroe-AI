...
class FullKelly(BaseKelly):
    """Full Kelly — no fractional discount, no regime multiplier."""
    def compute(self, params: KellyParameters) -> KellyResult:
        b = params.avg_win / params.avg_loss if params.avg_loss != 0 else 0
        p = params.win_rate
        q = 1 - p
        # ponytail: full Kelly, multiplier forced to 1.0 so legacy tests match f*=(bp-q)/b
        f_star = max(0, (b * p - q) / b) if b > 0 else 0
        f_star = min(f_star, params.leverage_max)
        g = self._growth_rate(f_star, p, b)
        return KellyResult(f_star=f_star, method=KellyMethod.FULL, growth_rate=g, parameters=params)
