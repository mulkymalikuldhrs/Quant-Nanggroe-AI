    ...
    with _em_lock:
        ...
        if allow_live:
            try:
                ...
                # Build a unified list: config accounts first, then discovered
                # accounts not already covered by config (dedup by login).
                wired = 0
                _wired_logins = set()
                ...
                # 2) discovered accounts (real terminals) — preferred, fills gaps
                for d in discovered:
                    _wire_account(
                        login=d.login,
