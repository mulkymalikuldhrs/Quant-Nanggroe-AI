"""Backward-compatibility shim — re-exports ``mean_reversion`` from archive."""
import importlib.util
import sys
from pathlib import Path

_ARCHIVE_FILE = Path(__file__).resolve().parent.parent.parent.parent.parent \
    / "archive" \u002f "strategies_legacy" \u002f "mean_reversion.py"

if _ARCHIVE_FILE.exists():
    spec = importlib.util.spec_from_file_location(
        f"quant_nanggroe.engine.strategy.strategies.mean_reversion",
        _ARCHIVE_FILE,
    )
    ...
    # Re-export names
    for _name in [MeanReversionStrategy]:
