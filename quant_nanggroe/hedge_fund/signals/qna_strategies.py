...
Each function imports a MUE-X gene strategy from E:/mue-x/genes/qna_strategies/
and calls its generate_signal() on historical data. ~240 variants grouped by type:

Strategy types:
- AlgebraStrategy_mut_* (Algebraic strategy, 25+ mutations)
- AMDXStrategy_mut_* (ADX-based, 8+ mutations)
- EMAADXStrategy_mut_* (EMA+ADX combo, 30+ mutations)
- FiboStrategy_mut_* (Fibonacci-based, 20+ mutations)
- MeanReversionStrategy_mut_* (Mean reversion, 30+ mutations)
- MSNRStrategy_mut_* (Mean-Std Normalized Reversion, 20+ mutations)
- QuarterlyTheoryStrategy_mut_* (Quarterly theory, 1 mutation)
- SMCStrategy_mut_* (Smart Money Concept, 20+ mutations)
- SMCStrategyOld_mut_* (Legacy SMC, 6 mutations)
- WyckoffStrategy_mut_* (Wyckoff, 25+ mutations)

Related sections in hedge_fund.py: lines 337-5848
"""
