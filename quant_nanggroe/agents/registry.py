...
class AgentFactory:
    ...
    def create_agent(
        ...
        **kwargs: Any,
        ...
        # Find the role for this agent name
        role = None
        for agent_role, agent_name in AgentRegistry._role_mapping.items():
            if agent_name == name:
                role = agent_role
                break

        if role is None:
            role = AgentRole.RESEARCHER  # Default fallback

        # Pass required BaseAgent arguments if not already in kwargs
        if "name" not in kwargs:
            kwargs["name"] = name
        if "role" not in kwargs:
            kwargs["role"] = role
        if "description" not in kwargs:
            kwargs["description"] = f"{name} agent"
