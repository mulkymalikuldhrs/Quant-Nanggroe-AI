...
def _mock_forex_data(pair, timeframe, lookback_days) -> dict:
    logger.warning("MOCK MODE: Returning hardcoded forex data for %s", pair)
    return {
        "pair": pair.upper(),
        "timeframe": timeframe,
...
def _mock_carry(base, quote, account_size) -> dict:
    logger.warning("MOCK MODE: Returning hardcoded carry analysis for %s/%s", base, quote)
    rates = {
        "USD": 5.25, "EUR": 4.50, "GBP": 5.25, "JPY": -0.10,
        "CHF": 1.75, "AUD": 4.35, "NZD": 5.50, "CAD": 5.00,
    }
    base_rate = rates.get(base.upper(), 3.0)
    quote_rate = rates.get(quote.upper(), 3.0)
...
def _mock_cbank(central_bank, upcoming_only) -> dict:
    logger.warning("MOCK MODE: Returning hardcoded central bank data for %s", central_bank)
    bank_names = {
        "FED": "Federal Reserve",
...
def fetch_forex_data(
    ...
    # Mock fallback
    if _MOCK_MODE:
        return json.dumps(_mock_forex_data(pair, timeframe, lookback_days), indent=2)

    raise RuntimeError(
        f"Cannot fetch forex data for {pair}: real engine unavailable and _MOCK_MODE=False. "
...
def analyze_carry(
    ...
    # Mock fallback
    if _MOCK_MODE:
        return json.dumps(_mock_carry(base_currency, quote_currency, account_size), indent=2)

    raise RuntimeError(
        f"Cannot analyze carry for {base_currency}/{quote_currency}: "
        "rates not available and _MOCK_MODE=False."
    )
...
def monitor_cbank(
    ...
    # Mock fallback
    if _MOCK_MODE:
        return json.dumps(_mock_cbank(central_bank, upcoming_only), indent=2)

    raise RuntimeError(
        f"Cannot monitor {central_bank}: real engine unavailable and _MOCK_MODE=False. "
        "Install required dependencies or set _MOCK_MODE=True."
    )
