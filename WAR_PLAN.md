# QNA WAR PLAN — v8.0.21 CANONICAL SSOT — Phase 5: Parallel Profile Orchestration
...
- **Version:** v8.0.21 CANONICAL SSOT (`qna.py:40` `__version__ = "8.0.18"`). `pyproject.toml` still 5.1.0 historical — code SSOT is `qna.py` + `CANONICAL.md`. No drift on SSOT.
...
- Version drift: none. v8.0.21 locked in `qna.py:40` + `CANONICAL.md:5` (SSOT). `pyproject.toml` 5.1.0 is historical, not SSOT.
