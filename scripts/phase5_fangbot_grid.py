...
def backtest(df, params, balance=10000.0):
    ...
    for i in range(len(data)):
        ...
        if position == 1:
            ...
            if price <= es:
                capital += (es - ep) / ep * capital
                trades.append((es - ep) / ep)
                position = 0; eq[i] = capital
            elif price >= et:
                capital += (et - ep) / ep * capital
                trades.append((et - ep) / ep)
                position = 0; eq[i] = capital
        elif position == -1:
            eq[i] = capital + (ep - price) / ep * capital
            if price >= es:
                capital += (ep - es) / ep * capital
                trades.append((ep - es) / ep)
                position = 0; eq[i] = capital
            elif price <= et:
                capital += (ep - et) / ep * capital
                trades.append((ep - et) / ep)
                position = 0; eq[i] = capital
    ...
    return {'sharpe': round(float(sharpe), 3), 'ret_pct': round(float(r), 2), 'dd_pct': round(float(dd), 2),
            'wr': round(float(wr), 1), 'trades': len(trades)}

def main():
    t0 = time.time()
    df = load(); logf = len(df)
    # Reduced grid: 2*2*2*2*2 = 32 configs
    grid = {
        'lookback': [17, 20],
        'atr_mult': [1.2, 1.5],
        'rr_min': [2.5, 3.0],
    ...
    res = []
    for idx, vals in enumerate(cfgs):
        ...
        try:
            ...
            if r['sharpe'] > 0.5 and r['ret_pct'] > 0 and r['dd_pct'] > -25:
                print(f"PASS [{idx}] {params} SR={r['sharpe']:.3f} R={r['ret_pct']:+.2f}% DD={r['dd_pct']:.1f}% WR={r['wr']:.1f}% TR={r['trades']}")
    ...
    out.write_text(json.dumps({'total': len(res), 'passing': len(passg), 'best': best, 'top': passg[:5]}, indent=2, default=str))
    print(f"best={best}")
    return best
